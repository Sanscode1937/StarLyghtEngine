...
class GPUFunctions
{
	...
	//not really asynchronous but what fucking ever lol
	public static function preloadTextureAsync(path:String, onComplete:Void->Void, texFormat:Context3DTextureFormat = BGRA, optimizeForRender:Bool = false,?_cachekey:String):Void
	{
		if (_cachekey == null)
			_cachekey = path;
		...
		for (tex in trackedTextures)
			if (tex.cacheKey == _cachekey)
			{
				trace('Texture $_cachekey already exists!');
				return;
			}

		Assets.loadBitmapData(path, false).onComplete(function(_bmp:BitmapData)
		{
...
public static function convertBitmap(_bmp:BitmapData,key:String = '')
	{
		var _texture = FlxG.stage.context3D.createTexture(_bmp.width, _bmp.height, BGRA, false);
		_texture.uploadFromBitmapData(_bmp);
		_bmp.dispose();
		_bmp.disposeImage();
		
		trackedTextures.push(new TexAsset(_texture,key));
		return BitmapData.fromTexture(_texture);

	}


public static function getATFonGPU(path:String, _width:Int, _height:Int, texFormat:Context3DTextureFormat = COMPRESSED_ALPHA, optimizeForRender:Bool = false,
